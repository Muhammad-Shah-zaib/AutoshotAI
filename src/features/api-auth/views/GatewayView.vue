...
<template>
  <div class="flex items-center justify-center min-h-[calc(100vh-190px)] relative animate-fade-in">
    ...
    <div
      class="absolute top-[30%] left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-[radial-gradient(circle,rgba(71,161,246,0.1)_0%,transparent_70%)] pointer-events-none">
    </div>

    <!-- Gateway Card -->
    <div
      class="w-full max-w-[460px] flex flex-col items-center gap-6 p-10 px-9 bg-[var(--color-surface-container)] border border-white/4 rounded-[var(--radius-xl)] relative z-[1] shadow-[0_20px_50px_-10px_rgba(0,0,0,0.5)]">
      <!-- Info Button -->
      <button
        class="absolute top-5 right-5 bg-transparent border border-[var(--color-border)] text-[var(--color-text-secondary)] py-1.5 px-3 rounded-[var(--radius-DEFAULT)] font-sans text-[12px] font-medium cursor-pointer flex items-center gap-1.5 transition-all duration-200 ease-out z-10 hover:border-[var(--color-border-hover)] hover:text-[var(--color-text-primary)] hover:bg-white/4"
        @click="router.push({ name: 'about' })" title="About AutoShot AI">
      ...
      <div class="text-center">
        <h1 class="text-[24px] font-semibold text-[var(--color-text-primary)] tracking-[-0.02em] mb-2">AutoShot-AI
          Studio</h1>
      ...
      <div class="w-full flex flex-col gap-2">
        ...
        <div
          class="flex bg-white/3 border border-[var(--color-border)] rounded-[var(--radius-DEFAULT)] overflow-hidden">
          <button
            class="flex-1 py-2 px-4 border-none  font-sans bg-primary text-[13px] font-medium cursor-pointer transition-all duration-200 ease-out"
            :class="provider === PROVIDERS.GEMINI ? 'bg-[var(--color-primary)] text-[var(--color-on-primary)] shadow-[0_4px_12px_rgba(71,161,255,0.2)]' : 'bg-transparent text-[var(--color-text-muted)] hover:text-[var(--color-text-secondary)] hover:bg-white/3'"
            @click="provider = PROVIDERS.GEMINI">
            Gemini
          </button>
          <button
            class="flex-1 py-2 px-4 border-none font-sans text-[13px] font-medium cursor-pointer transition-all duration-200 ease-out"
            :class="provider === PROVIDERS.OPENROUTER ? 'bg-[var(--color-primary)] text-[var(--color-on-primary)] shadow-[0_4px_12px_rgba(71,161,255,0.2)]' : 'bg-transparent text-[var(--color-text-muted)] hover:text-[var(--color-text-secondary)] hover:bg-white/3'"
            @click="provider = PROVIDERS.OPENROUTER">
            OpenRouter
          </button>
          <button
            class="flex-1 py-2 px-4 border-none font-sans text-[13px] font-medium cursor-pointer transition-all duration-200 ease-out"
            :class="provider === PROVIDERS.PROXY ? 'bg-[var(--color-primary)] text-[var(--color-on-primary)] shadow-[0_4px_12px_rgba(71,161,255,0.2)]' : 'bg-transparent text-[var(--color-text-muted)] hover:text-[var(--color-text-secondary)] hover:bg-white/3'"
      ...
      <Transition name="slide-fade">
        <div v-if="isProxy" class="w-full flex flex-col gap-2">
          ...
          <input id="proxy-url-input" type="url" v-model="proxyUrl" placeholder="http://localhost:4000"
            class="input-field" />
      ...
      <div class="w-full flex flex-col gap-2">
        ...
        <div class="relative">
          <input id="api-key-input" :type="showKey ? 'text' : 'password'" v-model="apiKey" :placeholder="keyPlaceholder"
            class="input-field pr-11" @keydown.enter="handleValidate" />
          <button
            ...
            @click="showKey = !showKey" :title="showKey ? 'Hide key' : 'Show key'" type="button">
      ...
      <div class="w-full flex flex-col gap-2">
        ...
        <div class="relative">
          <input id="store-key-input" :type="showStoreKey ? 'text' : 'password'" v-model="storeSecretKey"
            placeholder="For 'Push to Store' features..." class="input-field pr-11" @keydown.enter="handleValidate" />
          <button
            class="absolute right-2 top-1/2 -translate-y-1/2 bg-none border-none text-[var(--color-text-muted)] cursor-pointer p-1 flex items-center justify-center transition-colors duration-200 hover:text-[var(--color-text-secondary)]"
            @click="showStoreKey = !showStoreKey" :title="showStoreKey ? 'Hide key' : 'Show key'" type="button">
      ...
      <Transition name="slide-fade">
        <div v-if="validationError"
          class="flex items-center gap-2 w-full py-2.5 px-3.5 text-[13px] text-[var(--color-error)] bg-[rgba(147,0,10,0.1)] border border-[rgba(255,180,171,0.15)] rounded-[var(--radius-DEFAULT)]">
      ...
      <!-- Submit Button -->
      <button id="validate-key-btn" class="btn-primary w-full py-3 px-6 text-[15px] mt-1"
        :class="{ 'animate-pulse-glow': isValidating }" :disabled="!canSubmit" @click="handleValidate">
